...
function loadFullQuiz() {
    ...
    quizHTML += `
        ...
        <div style="display:flex; justify-content:flex-end; margin-top:15px;">
            <button id="next-btn" style="display:none;">Next: Simulation</button>
        </div>
    `;

    contentArea.innerHTML = quizHTML;

    // Update DOM references after adding Next button
...
// =======================
// When Next button on pre-quiz is clicked, go to simulation
// =======================
...
`;
    ...
    simulationscenarios.forEach((sc, idx) => {
        ...
        <div class="email unread" data-index="${idx}" style="padding:10px;border:1px solid #ccc;margin-bottom:8px;border-radius:6px;cursor:pointer;background:#fff3f3;">
            ...
            <p>${sc.content.substring(0, 80)}...</p>
        </div>
        `;
        } else {
            // Baiting & pretexting are just descriptions
            inboxHTML += `
        <div class="scenario-description" data-index="${idx}" style="padding:10px;border:1px solid #ccc;margin-bottom:8px;border-radius:6px;background:#f9f9f9;">
    ...
    // Add click listeners to emails
    document.querySelectorAll(".email").forEach(emailDiv => {
        emailDiv.addEventListener("click", () => {
            const idx = parseInt(emailDiv.dataset.index);
